...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, t, u, v, layers): # layers = [3, 20, 20, 20, 20, 20, 20, 20, 20, 2]
        ...
        # tf.contrib is not supported anymore and its causing problems 
        ...
        # self.optimizer = scipy.optimize.minimize(fun = self.loss,
        #                                          method = 'L-BFGS-B')
        ndims = 1000000 # N x T 
        start = tf.range(ndims, 0, -1, dtype="float64")

        self.optimizer = tfp.optimizer.lbfgs_minimize(self.loss, max_iterations=50000, initial_position=start, tolerance=1.0 * np.finfo(float).eps, 
                                                      num_correction_pairs=50, max_line_search_iterations = 50)
...
if __name__ == "__main__": 
    ...
    # Rearrange Data 
    XX = np.tile(X_star[:,0:1], (1,T)) # N x T, build a 1 x T matrix with all element in the first colum (x direction)
    YY = np.tile(X_star[:,1:2], (1,T)) # N x T  build a 1 x T matrix with all element in the second colum (y direction)
    TT = np.tile(t_star, (1,N)).T # N x T
    
    UU = U_star[:,0,:] # N x T(5000, 200) X dimension
    VV = U_star[:,1,:] # N x T(5000, 200) Y dimension
    PP = P_star # N x T
